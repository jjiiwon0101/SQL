...
-- employee_id가 103번인 사람과 job-id가 동일한 사람을 검색하는 문장.
SELECT job_id FROM employees WHERE employee_id = 103;


-- 다음 문장은 서부쿼리가 리턴하는 행이 여러개라서 단일행 연산자를 사용할 수 없습니다.
...
-- FIRST-NAME이 DAVID인 사람 중 가장 작은 값보다 급여가 큰 사람을 조회
...
-- 범위 지정도 불가능, 지목할 수 없는 문제가 발생하더라.
-- 이유: WHERE절부터 먼저 실행하고 나서 ROWNUM이 SELECT 되기 때문에.
-- 해결: ROWNUM까지 붙여 놓고 다시 한번 자료를 SELECT 해서 범위를 지정해야 되겠구나.
...
가장 바깥 쪽 select 절에서는 이미 붙어있는 rownum의 범위를 지정해서 조회.
